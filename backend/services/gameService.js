...
export async function createNewGame(userId, settings, answer) {
    const game = await Game.findOneAndUpdate({ userId, "settings.mode": settings.mode, gameState: "playing" }, { $set: { settings, answer, guesses: [], gameState: "playing", version: 0 } }, { new: true, upsert: true });
    if (!game) new EndpointError(404, "Game");
    return game;
}
